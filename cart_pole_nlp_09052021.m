...
t1=0; tf=5; t_list=linspace(t1,tf,n); h=(tf-t1)/(n-1); % Step size
...
%     v_list_guess=zeros(1,n);
%     for vv=1:(n-1)
%         v_list_guess(vv+1)=2/h*(x_list_guess(vv+1)-x_list_guess(vv))-v_list_guess(vv);
%     end
%     a_list_guess=zeros(1,n); a_list_guess(1)=6;
%     for aa=1:(n-1)
%         a_list_guess(aa+1)=2/h*(v_list_guess(aa+1)-v_list_guess(aa))-a_list_guess(aa);
%     end
...
%     omega_list_guess=zeros(1,n);
%     for oo=1:(n-1)
%         omega_list_guess(oo+1)=2/h*(theta_list_guess(oo+1)-theta_list_guess(oo))-omega_list_guess(oo);
%     end
%     alpha_list_guess=zeros(1,n); alpha_list_guess(1)=6*pi;
%     for ll=1:(n-1)
%         alpha_list_guess(ll+1)=2/h*(omega_list_guess(ll+1)-omega_list_guess(ll))-alpha_list_guess(ll);
%     end
...
% Optimization
options = optimoptions('fmincon','MaxFunctionEvaluations',100*n,'StepTolerance',1e-3);
