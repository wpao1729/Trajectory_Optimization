function [c,ceq] = standing_jump_nlcon(var_list,n,m_list,I_list,g,L_list,d_list,d_jump,h_jump,miu,tf,h)
...
for i=1:n % at every knot point
    ...
    if d_jump>=0
        h_terrain=h_jump*heaviside(P0i-(d_jump-0.1)+1e-6);
    elseif d_jump<0
        h_terrain=h_jump*(1-heaviside(P0i-(d_jump-0.1)+1e-6));
    end
    c=[c,(h_terrain-P0j)];
...
%     end
    ...
    if i<n1
        ...
        ceq=[ceq,P0i,P0j,vP0i,vP0j,aP0i,aP0j];
        c=[c,(1e-6-normal),(abs(friction)-miu*normal)];
    elseif (i>=n1)&&(i<=n2)
        % Aerial Phase constraints
        ceq=[ceq,friction,normal,u_list(1),0,0,0];
        c=[c,0,0];
    elseif i>n2
        % Contact phase2 constraints:
        ceq=[ceq,P0i-d_jump,P0j-h_jump,vP0i,vP0j,aP0i,aP0j];
        c=[c,(1e-6-normal),(abs(friction)-miu*normal)];
    end

    
%     % Collocation on center of mass for aerial phase and phase transition
%     if (i>=(n1-1))&&(i<=n2)
%         ceq_co1=sum(m_list.*G_mat(1:2:end,i+1)')-sum(m_list.*G_mat(1:2:end,i)')-h/2*(sum(m_list.*vG_mat(1:2:end,i)')+sum(m_list.*vG_mat(1:2:end,i+1)')); % Ci and vCi
%         ceq_co2=sum(m_list.*G_mat(2:2:end,i+1)')-sum(m_list.*G_mat(2:2:end,i)')-h/2*(sum(m_list.*vG_mat(2:2:end,i)')+sum(m_list.*vG_mat(2:2:end,i+1)')); % Cj and vCj
%         ceq_co3=sum(m_list.*vG_mat(1:2:end,i+1)')-sum(m_list.*vG_mat(1:2:end,i)')-h/2*(sum(m_list.*aG_mat(1:2:end,i)')+sum(m_list.*aG_mat(1:2:end,i+1)')); % vCi and aCi
%         ceq_co4=sum(m_list.*vG_mat(2:2:end,i+1)')-sum(m_list.*vG_mat(2:2:end,i)')-h/2*(sum(m_list.*aG_mat(2:2:end,i)')+sum(m_list.*aG_mat(2:2:end,i+1)')); % vCj and aCj
...
%     % Collocation on P0 but exclude collocation for aP0 at landing    
%     if i<=(n-1)
%         ceq_co1=P_mat(1,i+1)-P_mat(1,i)-h/2*(vP0_mat(1,i)+vP0_mat(1,i+1)); % P0i and vP0i
%         ceq_co2=P_mat(2,i+1)-P_mat(2,i)-h/2*(vP0_mat(2,i)+vP0_mat(2,i+1)); % P0j and vP0j
%         ceq_co3=vP0_mat(1,i+1)-vP0_mat(1,i)-h/2*(aP0_mat(1,i)+aP0_mat(1,i+1)); % vP0i and aP0i
%         ceq_co4=vP0_mat(2,i+1)-vP0_mat(2,i)-h/2*(aP0_mat(2,i)+aP0_mat(2,i+1)); % vP0j and aP0j
%         if i>(n2-1) && i<=n2
%             ceq=[ceq,ceq_co1,ceq_co2,0,0];
%         else
%             ceq=[ceq,ceq_co1,ceq_co2,ceq_co3,ceq_co4];
%         end
%     end
