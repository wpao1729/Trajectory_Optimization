...
t1=0; tf=1.3; d_jump=0.8; h_jump=0.8;
...
n_dv=42*n+2; % (+2 time variables for phase transition)
...
n_bc=22; % 2 for P5ij(n)=(D,0), 2 for P5ij(n-1)=P5ij(n)
...
% Aeq_bc(23:24,(27*n+5):(27*n+6))=diag(ones(1,2)); %beq=0 % initial vG3ij=0
% Aeq_bc(25:26,(33*n-1):(33*n))=diag(ones(1,2)); %beq=0 % final vG3ij=0
% Aeq_bc(27:28,(33*n+5):(33*n+6))=diag(ones(1,2)); %beq=0 % initial aG3ij=0
% Aeq_bc(29:30,(39*n-1):(39*n))=diag(ones(1,2)); %beq=0 % final aG3ij=0
% Aeq_bc(23:24,(42*n+1):(42*n+2))=diag(ones(1,2)); beq_bc(23:24)=[tf/2-sqrt((h_jump+0.4)/2/g),tf/2+sqrt((h_jump+0.4)/2/g)];
...
% A((4*n+1):5*n,3:3:3*n)=diag(ones(1,n)); b((4*n+1):5*n)=5/180*pi; % q3<=5 deg
% A((5*n+1):6*n,3:3:3*n)=diag(-1*ones(1,n)); b((5*n+1):6*n)=5/180*pi; % q3>=-5 deg

A(n_hu+1,42*n+1)=-1; b(n_hu+1)=-1e-6;
A(n_hu+2,42*n+2)=1; b(n_hu+2)=tf-1e-6;
A(n_hu+3,42*n+1)=1; A(n_hu+3,42*n+2)=-1; b(n_hu+3)=-1e-6;


% ------------------------------------------------------------------------

% Initialization
var_list_guess=initialize_standing_jump(n,qi_list,qf_list,L_list,d_list,m_list,I_list,g,d_jump,h_jump,tf);

% Optimization with fmincon
func_cost=@(var_list)standing_jump_cost(var_list,n,tf);
func_nlcon=@(var_list)standing_jump_nlcon(var_list,n,m_list,I_list,g,L_list,d_list,d_jump,h_jump,miu,tf,h);
...
for k=1:n
    ...
    %axis([d_jump-0.5,0.5,0,h_jump+1.6]);
    dt=tf/n/1.2; % Finish animation in tf
    pause(dt);
    if k==1
        pause(0.5);
    end
